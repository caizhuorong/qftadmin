...
const okHandle = () => {
  ...
  return (
    ...
    >
      ...
      <FormItem labelCol={{ span: 7 }} wrapperCol={{ span: 15 }} label="单次领取数量">
      ...
      <FormItem labelCol={{ span: 7 }} wrapperCol={{ span: 15 }} label="可领取次数">
        {form.getFieldDecorator('descssss', {
          rules: [{ required: true, message: '请输入可领取次数' }],
          initialValue:1
        })(<Input placeholder="请输入"  disabled/>)}
      </FormItem>
       <FormItem labelCol={{ span: 7 }} wrapperCol={{ span: 15 }} label="完成任务周期奖励">
        {form.getFieldDecorator('cyclesReward', {
          rules: [{ required: true, message: '请输入完成任务周期奖励' }],
          initialValue:top.cyclesReward
        })(<Input placeholder="请输入完成任务周期奖励" />)}
      </FormItem>
    </Modal>
  );
});
...
export default class TableList extends PureComponent {
  ...
  handleAdd = fields => {
    const { dispatch } = this.props;
 		 dispatch({
      type: 'signin/editTask',
      payload:{
    		id:this.state.taskId,
		  	activate:fields.activate,
		  	token:fields.token,
		  	cyclesReward:fields.cyclesReward
      },
      callback(){
	...
	man_handleChange= (value)=>{
		...
		if( value == 0 || value == 1 ){
				this.setState({
					type:value,
					taskId:2,
					type_text:value == 1 ? '发放数值':'参与人数',
					type_text1:value == 1 ? '该时间段发放数值量':'该时间段参与人数量',
				},()=>{
					dispatch({
			      type: 'signin/chart',
			      payload:{
			...
			this.setState({
					type:value,
					taskId:12,
					type_text:value == 2 ? '完成任务周期人数':'完成任务周期发放数值',
					type_text1:value == 2 ? '该时间段完成任务周期人数量':'该时间段完成任务周期发放量',
				},()=>{
					dispatch({
			      type: 'signin/chart',
			      payload:{
  ...
  render() {
  	const {signin:{data,chart_data,top,top_toal,zg},loading} = this.props;
      	...
      	<Card bordered={false}>
      		<Tabs defaultActiveKey="1" onChange={this.callback.bind(this)}>
				    <TabPane tab="列表" key="1">
			          ...
			          <Description term="单日可领取">1</Description>
			          <Description term="单次领取数量">{top.token}</Description>
			          <Description term="完成任务周期奖励">{top.cyclesReward}</Description>
			        </DescriptionList>
			        <StandardTable
				    	...
				    	<Row gutter={24}>
					      <Col span={10}>
				      		<Select defaultValue="1" style={{width:'30%'}} onChange={this.man_handleChange}>
							      ...
							      <Option value="2">完成任务周期人数</Option>
							      <Option value="3">完成任务周期发放数值</Option>
							    </Select>
					      </Col>
					      <Col span={7} offset={7}>
					      		{datediv}
